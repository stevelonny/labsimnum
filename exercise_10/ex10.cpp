...
int main(int argc, char* argv[]){
    ...
    if(argc >=2 && strcmp(argv[1], "circle")==0){
        ...
        if(rank == 0) fmt::print(cerr, "Usage: {0} <mapper> [<seltype>]\nwhere <mapper> is either 'circle' or 'square' or 'provita'\n", argv[0]);
        if(rank == 0) fmt::print(cerr, "and optionally <seltype> is either 0(lin rank-based) 1(roulette) 2(tournament) 3(stoch-univ-acc)\n");
        MPI_Finalize();
        return 1;
        }
    else{
