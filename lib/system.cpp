...
System :: System(){
  _path_input = paths::path_IO / "INPUT";
  _path_output = paths::path_IO / "OUTPUT";
}

System :: System(std::filesystem::path input_path, std::filesystem::path output_path){
  _path_input = input_path;
  _path_output = output_path;
}
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  ifstream input(_path_input / "input.dat"); // Start reading ../INPUT/input.dat
  ofstream coutf;
...
void System :: initialize(system_input input){
  ...
  _nblocks = input.nblocks;
  coutf << "NBLOCKS= " << _nblocks << endl;
  _nsteps = input.nsteps;
  coutf << "NSTEPS= " << _nsteps << endl;
  coutf << "Reading input from struct completed!" << endl;
  this->read_configuration();
  this->initialize_velocities();
  coutf << "System initialized!" << endl;
  coutf.close();
...
void System :: initialize_velocities(){
  if(_restart and _sim_type==0){
    ifstream cinf;
    cinf.open(_path_input / "CONFIG/velocities.in");
    if(cinf.is_open()){
      double vx, vy, vz;
      for(int i=0; i<_npart; i++){
        cinf >> vx >> vy >> vz;
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  ifstream input(_path_input / "properties.dat");
  if (input.is_open()){
    while ( !input.eof() ){
      input >> property;
      if( property == "POTENTIAL_ENERGY" ){
        ...
        ofstream coutf;
        coutf.open(_path_output / "output.dat",ios::app);
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(_path_output / "CONFIG/config.xyz");
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    coutf.open(_path_output / "CONFIG/config.spin");
...
void System :: write_velocities(){
  ofstream coutf;
  coutf.open(_path_output / "CONFIG/velocities.out");
  if(coutf.is_open()){
    for(int i=0; i<_npart; i++){
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open(_path_input / "CONFIG/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
  ...
  if(_restart and _sim_type > 1){
    int spin;
    cinf.open(_path_input / "CONFIG/config.spin");
    for(int i=0; i<_npart; i++){
      cinf >> spin;
...
void System :: measure(){ // Measure properties
  ...
  double virial_temp=0.0; // temporary accumulator for virial
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open(_path_output / "potential_energy.dat",ios::app);
  ...
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
    coutf.open(_path_output / "kinetic_energy.dat",ios::app);
  ...
  // TOTAL ENERGY //////////////////////////////////////////////////////////////
  if (_measure_tenergy){
    coutf.open(_path_output / "total_energy.dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open(_path_output / "temperature.dat",ios::app);
